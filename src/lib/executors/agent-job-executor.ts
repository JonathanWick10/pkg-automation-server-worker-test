...
import { AgentJobData } from 'automation-server-shared-test'
import { Agent, agentbuiltInToolsNames, AgentStepBlock, AgentTaskStatus, assertNotNullOrUndefined, ContentBlockType, isNil, McpToolType, McpWithTools, ToolCallContentBlock, ToolCallStatus, ToolCallType, UpdateAgentRunRequestBody } from 'automation-shared-test'
...
const {AIUsageFeature, createAIModel} = commonai;
            ...
            for await (const chunk of fullStream) {
                if (chunk.type === 'text-delta') {
                    ...
                    if (APICallError.isInstance(chunk.error)) {
                        const errorResponse = (chunk.error as unknown as { data: commonai.AIErrorResponse })?.data
