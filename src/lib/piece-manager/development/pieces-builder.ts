...
import { ApLock, CacheState, filePiecesUtils, GLOBAL_CACHE_COMMON_PATH, memoryLock, PiecesSource } from 'automation-server-shared-test'
import { debounce, isNil, WebsocketClientEvent } from 'automation-shared-test'
...
async function handleFileChange(packages: string[], pieceProjectName: string, piecePackageName: string, nxProjectFilePath: string, io: Server, log: FastifyBaseLogger): Promise<void> {
    ...
    try {
        ...
        await cache.setCache('automation-pieces-framework-test', CacheState.PENDING)
        await cache.setCache('@activepieces/pieces-common', CacheState.PENDING)
        await cache.setCache('automation-shared-test', CacheState.PENDING)
